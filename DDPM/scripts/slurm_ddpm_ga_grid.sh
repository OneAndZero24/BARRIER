...
# Fixed:  method=ga, targets=QKV+cemb, use_actual_bounds=true,
...
cfg["wandb"]["group"] = "cifar10-ga-qkv-cemb-fullsweep"
cfg["wandb"]["tags"] = list(cfg["wandb"].get("tags", [])) + [
    "ga", "fullsweep", "qkv-classemb",
    f"lr_{lr}", f"niters_{niters}", f"lambda_{lam}", f"rdim_{reduced_dim}",
]

suffix = f"ga_lr{lr}_ni{niters}_lam{lam}_rdim{reduced_dim}"
...
echo "GA grid job ${IDX} (lr=${LR}, n_iters=${NITERS}, lambda=${LAMBDA}, rdim=${REDUCED_DIM}) complete."
