...
def compute_fid(real_image_paths: List[str], fake_image_paths: List[str],
    ...
    try:
        ...
        log.warning(f"Cannot compute FID: {e}")
        return None
    ...
    # ToTensor produces [0,1], FrechetInceptionDistance expects uint8 [0,255]
...
def _update_fid_from_paths(fid, paths, transform, real: bool, batch_size: int = 64):
    """Feed images to FID in batches to keep memory usage bounded."""
    count = 0
    buffer = []
    for path in paths:
        try:
            buffer.append(transform(Image.open(path)))
        except Exception as exc:
            log.warning(f"Failed to load {path}: {exc}")
            continue

        if len(buffer) >= batch_size:
            batch = (torch.stack(buffer) * 255).clamp(0, 255).to(torch.uint8)
...
def _update_fid_from_hf_dataset(fid, ds, idxs, transform, real: bool, batch_size: int = 64):
    """Feed images from a HuggingFace dataset to FID in batches."""
    count = 0
    buffer = []
    for idx in idxs:
        try:
            buffer.append(transform(ds[int(idx)]["image"]))
        except Exception as exc:
            log.warning(f"Failed to load COCO sample {idx}: {exc}")
            continue
...
def _load_coco_hf_dataset(dataset_name: str):
    """Load the COCO HF dataset, trying the common validation splits in order."""
    from datasets import load_dataset

    last_exc = None
    for split_name in ("test", "train"):
        try:
            log.info("Loading COCO images from HuggingFace dataset %s split=%s", dataset_name, split_name)
            return load_dataset(dataset_name, split=split_name)
        except Exception as exc:
            last_exc = exc
            log.warning("Could not load %s split=%s: %s", dataset_name, split_name, exc)
    raise last_exc
...
def compute_fid_coco(
    ...
    - Otherwise fall back to a HuggingFace COCO validation dataset.
    - Images are fed to the metric in batches to keep memory bounded.
    """
    try:
        from torchmetrics.image.fid import FrechetInceptionDistance
    except ImportError as e:
    ...
    if coco_hf_dataset:
        try:
            ...
            ds = _load_coco_hf_dataset(coco_hf_dataset)
            idxs = rng.choice(len(ds), min(n, len(ds)), replace=False)
            if max_real and len(idxs) > max_real:
                idxs = rng.choice(idxs, max_real, replace=False)
            n_real = _update_fid_from_hf_dataset(fid, ds, idxs, transform, real=True, batch_size=batch_size)
            del ds
        except Exception as exc:
            log.warning(f"Cannot load HF dataset {coco_hf_dataset}: {exc}")
            return None
        ...
        if max_real and len(all_real) > max_real:
            ...
            all_real = [all_real[i] for i in idxs]
        n_real = _update_fid_from_paths(fid, all_real, transform, real=True, batch_size=batch_size)
    else:
        try:
            log.info("Loading COCO images from HuggingFace for FID …")
            ds = _load_coco_hf_dataset("sayakpaul/coco-30-val-2014")
            k = min(n, len(ds))
            ...
            log.warning(f"Cannot load COCO images: {exc}")
            return None
    ...
    if max_fake and len(gen_paths) > max_fake:
        idxs = rng.choice(len(gen_paths), max_fake, replace=False)
        gen_paths = [gen_paths[i] for i in idxs]

    n_fake = _update_fid_from_paths(fid, gen_paths, transform, real=False, batch_size=batch_size)
    if n_fake == 0:
        log.warning("No generated images found for FID")
        return None

    score = fid.compute().item()
    log.info(f"FID (COCO) = {score:.2f} (real={n_real}, fake={n_fake})")
    return score
...
def compute_ua_concept(images_dir: str, concept_prompts: List[str],
    ...
    for img_path in all_images:
        try:
            ...
            with torch.no_grad():
                outputs = model(**inputs)
                scores.append(outputs.logits_per_image.max().item())
        except Exception:
            continue
