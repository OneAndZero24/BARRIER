...
def savemodelDiffusers(name, path, compvis_config_file, diffusers_config_file, extract_ema, device='cuda'):
    checkpoint_path = f'{path}/{name}.ckpt'
    ...
    # extract_ema = False
    dump_path = f"{path}/{name.replace('compvis','diffusers')}-ema{extract_ema}.pt"
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--name', type=str, default='nudity')
    argparser.add_argument('--path', type=str, required=True)
    argparser.add_argument('--compvis_config_file', type=str, default='/vhome/gongchao/UCE_acceleration/adv-train/utils/convert_models/v1-inference.yaml')
    argparser.add_argument('--diffusers_config_file', type=str, default=None)
    argparser.add_argument('--extract_ema', action='store_true')

    args = argparser.parse_args()
    savemodelDiffusers(args.name, args.path, args.compvis_config_file, diffusers_config_file=args.diffusers_config_file, extract_ema=args.extract_ema, device='cuda')