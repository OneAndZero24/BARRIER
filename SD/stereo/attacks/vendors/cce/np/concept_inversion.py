...
def main():
    ...
    assert args.guidance_scale > 0.0, "Guidance scale must be greater than 0.0"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                if args.i2p:
                    tokens = [""]*bsz + batch["text_original"] + [args.safety_concept]*bsz
                else:
                    tokens = [""]*bsz + [args.safety_concept]*bsz
                ...
                if args.i2p:
                    noisy_latents = torch.cat([noisy_latents] * 4)
                    timesteps = torch.cat([timesteps] * 4)
                else:
                    noisy_latents = torch.cat([noisy_latents] * 3)
                    timesteps = torch.cat([timesteps] * 3)
