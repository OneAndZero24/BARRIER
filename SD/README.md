...
BARRIER (Interval Protection) unlearning adds protection loss to preserve model performance on retain data while unlearning forget concepts. It is composable with all existing base methods (GA, RL, NSFW, ESD).

## Usage

BARRIER targets cross-attention Q/K/V projections (`to_q`, `to_k`, `to_v`) by default. You can customize targets via `--targets`.

1. **BARRIER + Gradient Ascent (class forgetting)**:
   ```bash
...
2. **BARRIER + Random Label (class forgetting)**:
   ```bash
   python train-scripts/intact_unlearn.py \
       --base_method rl \
       --class_to_forget 0 \
...
| `--lambda_interval` | `1.0` | Weight for BARRIER protection loss |
